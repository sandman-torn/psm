...
import useAuth from '@/composable/useAuthPenganjur' // adjust if @ doesn't work

//const { user, currentRole } = useAuth()

const currentRole = defineProps(['role'])
console.log('Current now Role:', currentRole.role)
  ...
  return menuGroups
    .map((group) => {
      const visibleItems = group.items.filter((item) => {
        ...
        return item.roles.includes(currentRole.role)
      })
